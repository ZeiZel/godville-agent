...
export type LiveCommandId = "hero.encourage" | "hero.punish" | "arena.voice.heal" | "arena.voice.attack" | "hero.restore_prana" | "adventure.dungeon.start" | "adventure.polygon.start" | "arena.zpg.start" | "dungeon.move.auto" | "polygon.move.safe";
...
export const LIVE_COMMANDS: Readonly<Record<LiveCommandId, ReviewedLiveCommand>> = {
  "hero.encourage": { id: "hero.encourage", maxPrana: 25, maxCharges: 0, allowedModes: ["idle", "dungeon", "arena"] },
  "hero.punish": { id: "hero.punish", maxPrana: 25, maxCharges: 0, allowedModes: ["arena"] },
  "arena.voice.heal": { id: "arena.voice.heal", maxPrana: 5, maxCharges: 0, allowedModes: ["arena"] },
  "arena.voice.attack": { id: "arena.voice.attack", maxPrana: 5, maxCharges: 0, allowedModes: ["arena"] },
...
export const GODVILLE_ENCOURAGE_TARGET_EXPRESSION = `(() => {const label='Сделать хорошо',links=[...document.querySelectorAll('#control a.enc_link.div_link,#cntrl a.enc_link,#m_control a.enc_link.div_link')].filter(a=>(a.innerText||'').trim()===label);if(location.origin!=='https://godville.net'||location.pathname!=='/superhero'||links.length!==1)return JSON.stringify({ready:false});const a=links[0],r=a.getBoundingClientRect(),s=getComputedStyle(a),cx=r.left+r.width/2,cy=r.top+r.height/2,hit=document.elementFromPoint(cx,cy);const enabled=!a.hasAttribute('disabled')&&a.getAttribute('aria-disabled')!=='true'&&s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity)>0&&r.width>4&&r.height>4&&!!hit&&(hit===a||a.contains(hit));return JSON.stringify({ready:enabled,x:Math.round(r.left),y:Math.round(r.top),width:Math.round(r.width),height:Math.round(r.height)})})()`;
/** After pointer movement this fixed expression verifies that the exact target is hovered. */
export const GODVILLE_ENCOURAGE_HOVER_EXPRESSION = `(() => {const label='Сделать хорошо',links=[...document.querySelectorAll('#control a.enc_link.div_link,#cntrl a.enc_link,#m_control a.enc_link.div_link')].filter(a=>(a.innerText||'').trim()===label);return JSON.stringify({ready:location.origin==='https://godville.net'&&location.pathname==='/superhero'&&links.length===1&&links[0].matches(':hover')})})()`;
export const GODVILLE_PUNISH_TARGET_EXPRESSION = `(() => {const label='Сделать плохо',links=[...document.querySelectorAll('#m_control a.pun_link.div_link')].filter(a=>(a.innerText||'').trim()===label);if(location.origin!=='https://godville.net'||location.pathname!=='/superhero'||links.length!==1)return JSON.stringify({ready:false});const a=links[0],r=a.getBoundingClientRect(),s=getComputedStyle(a),hit=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);return JSON.stringify({ready:!a.hasAttribute('disabled')&&a.getAttribute('aria-disabled')!=='true'&&s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity)>0&&r.width>4&&r.height>4&&!!hit&&(hit===a||a.contains(hit)),x:Math.round(r.left),y:Math.round(r.top),width:Math.round(r.width),height:Math.round(r.height)})})()`;
export const GODVILLE_PUNISH_HOVER_EXPRESSION = `(() => {const links=[...document.querySelectorAll('#m_control a.pun_link.div_link')].filter(a=>(a.innerText||'').trim()==='Сделать плохо');return JSON.stringify({ready:links.length===1&&links[0].matches(':hover')})})()`;
export const GODVILLE_ARENA_RECEIPT_EXPRESSION = `(() => {const e=document.querySelector('#v2l'),visible=e&&getComputedStyle(e).display!=='none'&&getComputedStyle(e).visibility!=='hidden';return JSON.stringify({success:!!visible&&!e.classList.contains('t_error')&&(e.innerText||'').trim().length>0})})()`;
export const GODVILLE_ARENA_VOICE_TARGET_EXPRESSION = `(() => {const input=document.querySelector('textarea#godvoice.input_text'),submit=document.querySelector('input#voice_submit');if(!input||!submit||input.readOnly||submit.disabled)return JSON.stringify({ready:false});const r=submit.getBoundingClientRect(),s=getComputedStyle(submit),hit=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);return JSON.stringify({ready:s.display!=='none'&&s.visibility!=='hidden'&&r.width>4&&r.height>4&&!!hit&&(hit===submit||submit.contains(hit)),x:Math.round(r.left),y:Math.round(r.top),width:Math.round(r.width),height:Math.round(r.height)})})()`;
export const GODVILLE_ARENA_VOICE_HOVER_EXPRESSION = `(() => {const submit=document.querySelector('input#voice_submit');return JSON.stringify({ready:!!submit&&!submit.disabled&&submit.matches(':hover')})})()`;
function arenaVoicePrepareExpression(phrase: "Лечись!" | "Бей!"): string { return `(() => {const input=document.querySelector('textarea#godvoice.input_text'),submit=document.querySelector('input#voice_submit');if(!input||!submit||input.readOnly||submit.disabled||(input.value||'').trim()!=='')return JSON.stringify({ready:false});input.value=${JSON.stringify(phrase)};input.dispatchEvent(new Event('input',{bubbles:true}));return JSON.stringify({ready:input.value===${JSON.stringify(phrase)}})})()`; }
function arenaVoicePreparedExpression(phrase: "Лечись!" | "Бей!"): string { return `(() => {const input=document.querySelector('textarea#godvoice.input_text'),submit=document.querySelector('input#voice_submit');return JSON.stringify({ready:!!input&&!!submit&&!input.readOnly&&!submit.disabled&&input.value===${JSON.stringify(phrase)}})})()`; }
function arenaVoiceClearExpression(phrase: "Лечись!" | "Бей!"): string { return `(() => {const input=document.querySelector('textarea#godvoice.input_text');if(!input||input.readOnly||input.value!==${JSON.stringify(phrase)})return JSON.stringify({cleared:false});input.value='';input.dispatchEvent(new Event('input',{bubbles:true}));return JSON.stringify({cleared:true})})()`; }
...
export const GODVILLE_ARENA_STATE_EXPRESSION = `(() => {const visible=e=>{if(!e)return false;for(let n=e,i=0;n&&i<5;n=n.parentElement,i++){const s=getComputedStyle(n);if(s.display==='none'||s.visibility==='hidden'||Number(s.opacity)===0)return false}const r=e.getBoundingClientRect();return r.width>0&&r.height>0};const enabled=e=>visible(e)&&!e.hasAttribute('disabled')&&e.getAttribute('aria-disabled')!=='true';const heading=(document.querySelector('#m_fight_log .block_title')?.innerText||'').trim(),turn=heading.match(/^Вести с арены\\s*\\(шаг\\s+(\\d+)\\)$/u),href=(document.querySelector('#fbclink')||{}).getAttribute?.('href')||null,op=(document.querySelector('#o_hl1')?.innerText||'').match(/(\\d+)\\s*\\/\\s*(\\d+)/u),active=!!turn&&typeof href==='string'&&/^\\/duels\\/log\\/[a-z0-9]+$/i.test(href)&&['#m_info','#o_info','#m_fight_log','#m_control'].every(s=>visible(document.querySelector(s))),blocked=[...document.querySelectorAll('.r_blocked')].filter(visible).map(e=>(e.innerText||'').trim()),terminal=blocked.includes('Для вашего героя бой окончен'),frozen=blocked.some(t=>/^Первые два хода пульты богов заморожены/u.test(t)),ordinary=[...document.querySelectorAll('#m_fight_log .d_line,#m_fight_log .d_msg')].some(e=>/аренные жрецы постараются не дать богам распаковать больше трех пранозарядов/u.test(e.innerText||'')),influenceReady=[...document.querySelectorAll('#m_control a.enc_link.div_link,#m_control a.pun_link.div_link')].some(enabled),voiceInput=document.querySelector('textarea#godvoice.input_text'),voiceSubmit=document.querySelector('input#voice_submit'),voiceReady=enabled(voiceInput)&&enabled(voiceSubmit)&&!voiceInput.readOnly&&(voiceInput.value||'').trim()==='';return JSON.stringify({origin:location.origin,path:location.pathname,active,terminal:active&&terminal,frozen:active&&frozen,ordinary:active&&ordinary,influenceReady:active&&influenceReady,voiceReady:active&&voiceReady,opponentHealthPercent:op&&Number(op[2])>0?Math.floor(100*Number(op[1])/Number(op[2])):null,turn:turn?Number(turn[1]):null,battleId:active?href:null})})()`;
...
function parseArenaFallback(value: Record<string, unknown>, arena: Record<string, unknown>, observedAt: string, heroId?: string): { observation: ObservationV1; diaryFingerprint: number; arenaTurn?: number; battleId?: string } | undefined {
  const health = Array.isArray(value.health) && value.health.length === 2 && value.health.every(nonnegativeInteger) ? value.health as [number, number] : undefined;
  const prana = nonnegativeInteger(value.pranaPercent) && value.pranaPercent <= 100 ? value.pranaPercent : undefined;
  const charges = nonnegativeInteger(value.charges) ? value.charges : undefined;
  const battleId = typeof arena.battleId === "string" && /^\/duels\/log\/[a-z0-9]+$/i.test(arena.battleId) ? arena.battleId : undefined;
  const arenaTurn = nonnegativeInteger(arena.turn) ? arena.turn : undefined;
  const opponentHealthPercent = nonnegativeInteger(arena.opponentHealthPercent) && arena.opponentHealthPercent <= 100 ? arena.opponentHealthPercent : undefined;
  if (arena.active !== true || value.origin !== GODVILLE_ORIGIN || value.path !== "/superhero" || !health || health[1] <= 0 || health[0] > health[1] || prana === undefined || charges === undefined || !battleId || arenaTurn === undefined || !nonnegativeInteger(value.diaryFingerprint)) return undefined;
  return { observation: { version: OBSERVATION_VERSION, observedAt, sourceVersion: "godville-dom/v1", freshness: "fresh", ...(heroId ? { heroId } : {}), battleId, mode: "arena", capabilities: [], progressionKnown: false, prana: { current: prana, capacity: 100 }, charges, health: health[0] / health[1] >= 0.5 ? "known_safe" : "known_risk", healthPercent: Math.floor(100 * health[0] / health[1]), ...(opponentHealthPercent === undefined ? {} : { opponentHealthPercent }), cooldowns: {}, rawShape: ["godville-superhero", "arena-active", `arena-turn:${arenaTurn}`, ...(arena.ordinary === true ? ["arena-ordinary-three-charge"] : []), ...(arena.influenceReady === true ? ["arena-influence-ready"] : []), ...(arena.voiceReady === true ? ["arena-voice-ready"] : []), ...(arena.frozen === true ? ["arena-frozen"] : []), ...(arena.terminal === true ? ["arena-terminal"] : [])] }, diaryFingerprint: value.diaryFingerprint, arenaTurn, battleId };
}
...
function commandDom(command: ReviewedLiveCommand): { target: string; hover: string } {
  ...
  if (command.id === "hero.punish") return { target: GODVILLE_PUNISH_TARGET_EXPRESSION, hover: GODVILLE_PUNISH_HOVER_EXPRESSION };
  if (command.id === "arena.voice.heal" || command.id === "arena.voice.attack") return { target: GODVILLE_ARENA_VOICE_TARGET_EXPRESSION, hover: GODVILLE_ARENA_VOICE_HOVER_EXPRESSION };
...
class OrcaGodvilleAdapter implements LiveBrowserAdapter {
  ...
  private async snapshot(): Promise<{ observation: ObservationV1; diaryFingerprint: number; dungeonTurn?: number; polygonTurn?: number; arenaTurn?: number; battleId?: string }> {
    const raw = await this.evaluate(GODVILLE_OBSERVE_EXPRESSION);
    let snapshot: { observation: ObservationV1; diaryFingerprint: number; dungeonTurn?: number; polygonTurn?: number; arenaTurn?: number; battleId?: string };
    ...
    if (snapshot.observation.mode === "unknown") {
      try {
        const arena = await this.evaluate(GODVILLE_ARENA_STATE_EXPRESSION);
        if (arena.active === true && nonnegativeInteger(arena.turn) && typeof arena.battleId === "string" && /^\/duels\/log\/[a-z0-9]+$/i.test(arena.battleId)) {
          snapshot.arenaTurn = arena.turn; snapshot.battleId = arena.battleId;
          snapshot.observation = { ...snapshot.observation, battleId: arena.battleId, mode: "arena", rawShape: [...snapshot.observation.rawShape, "arena-active", `arena-turn:${arena.turn}`, ...(arena.ordinary === true ? ["arena-ordinary-three-charge"] : []), ...(arena.influenceReady === true ? ["arena-influence-ready"] : []), ...(arena.voiceReady === true ? ["arena-voice-ready"] : []), ...(arena.frozen === true ? ["arena-frozen"] : []), ...(arena.terminal === true ? ["arena-terminal"] : [])] };
        }
    ...
    const eventFingerprint = snapshot.dungeonTurn !== undefined ? `dungeon-turn:${snapshot.battleId!}:${snapshot.dungeonTurn}` : snapshot.polygonTurn !== undefined ? `polygon-turn:${snapshot.battleId!}:${snapshot.polygonTurn}` : snapshot.arenaTurn !== undefined ? `arena-turn:${snapshot.battleId!}:${snapshot.arenaTurn}` : `diary:${snapshot.diaryFingerprint}`;
    if (this.stableEventFingerprint === eventFingerprint) snapshot.observation = { ...snapshot.observation, eventId: snapshot.dungeonTurn !== undefined || snapshot.polygonTurn !== undefined || snapshot.arenaTurn !== undefined ? `godville-${eventFingerprint}` : `godville-diary-fnv1a32:${snapshot.diaryFingerprint}` };
  ...
  async execute(commandId: LiveCommandId, options: LiveExecuteOptions): Promise<ClickOutcome> {
    ...
    let preparedVoice: "Лечись!" | "Бей!" | undefined;
    try {
    ...
    try {
      if (command.id === "arena.voice.heal" || command.id === "arena.voice.attack") {
        const phrase = command.id === "arena.voice.heal" ? "Лечись!" as const : "Бей!" as const;
        if ((await this.evaluate(arenaVoicePrepareExpression(phrase))).ready !== true) return this.finish(options, { state: "SKIPPED", clicked: false, confirmed: false, ambiguous: false, reason: "reviewed arena voice form changed after journal gate" });
        preparedVoice = phrase;
        const state = await this.evaluate(GODVILLE_ARENA_STATE_EXPRESSION);
        const turnMatch = finalSnapshot.observation.eventId?.match(/^godville-arena-turn:\/duels\/log\/[a-z0-9]+:(\d+)$/i);
        if ((await this.evaluate(arenaVoicePreparedExpression(phrase))).ready !== true || state.origin !== GODVILLE_ORIGIN || state.path !== "/superhero" || state.active !== true || state.frozen === true || state.terminal === true || state.battleId !== finalSnapshot.observation.battleId || !turnMatch || state.turn !== Number(turnMatch[1])) { await this.evaluate(arenaVoiceClearExpression(phrase)); return this.finish(options, { state: "SKIPPED", clicked: false, confirmed: false, ambiguous: false, reason: "arena battle turn or reviewed voice phrase changed before physical click" }); }
      }
      ...
      if (!rebound || rebound.x !== target.x || rebound.y !== target.y || rebound.width !== target.width || rebound.height !== target.height || (await this.evaluate(dom.hover)).ready !== true) {
        if (preparedVoice) await this.evaluate(arenaVoiceClearExpression(preparedVoice));
    ...
    } catch { if (preparedVoice) { try { await this.evaluate(arenaVoiceClearExpression(preparedVoice)); } catch { /* draft remains protected */ } } return this.finish(options, { state: "SKIPPED", clicked: false, confirmed: false, ambiguous: false, reason: "exact target could not be rebound after journal gate" }); }
    ...
    while (this.clock.now().getTime() - started <= POSTCONDITION_TIMEOUT_MS) {
      try {
        ...
        const arenaReceipt = after.observation.mode === "arena" ? (await this.evaluate(GODVILLE_ARENA_RECEIPT_EXPRESSION)).success === true : false;
        const encouraged = command.id === "hero.encourage" && beforePrana - afterPrana === 25 && (after.diaryFingerprint !== finalSnapshot.diaryFingerprint || (after.observation.mode === "dungeon" && after.observation.eventId?.startsWith(`godville-dungeon-turn:${dungeonBattleId}:`) === true) || (after.observation.mode === "arena" && after.observation.battleId === finalSnapshot.observation.battleId));
        const punished = command.id === "hero.punish" && after.observation.mode === "arena" && after.observation.battleId === finalSnapshot.observation.battleId && beforePrana - afterPrana === 25;
        const voiced = (command.id === "arena.voice.heal" || command.id === "arena.voice.attack") && after.observation.mode === "arena" && after.observation.battleId === finalSnapshot.observation.battleId && beforePrana - afterPrana === 5 && arenaReceipt;
        ...
        if (encouraged || punished || voiced || restored || dungeonStarted || dungeonMoved || polygonMoved) {
          await restorePolygonPrompt();
          return this.finish(options, { state: "CONFIRMED", clicked: true, confirmed: true, ambiguous: false, reason: restored ? "one accumulator charge and exact bounded prana restoration confirm recharge" : dungeonMoved ? "same-battle five-prana debit confirms the reviewed legal grid movement" : polygonMoved ? "same-polygon fifteen-prana debit confirms the reviewed adjacent push" : voiced ? "same-arena five-prana debit and accepted voice receipt confirm the reviewed voice" : punished ? "same-arena twenty-five-prana debit confirms the punishment request" : dungeonStarted ? "new non-field state, diary event, and reviewed dungeon-entry debit confirm launch" : "fresh diary event and bounded prana change confirm the influence", observation: after.observation });
        }
      } catch {
