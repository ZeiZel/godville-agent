...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    #timeline { max-height:520px; overflow:auto; padding-right:4px; } .action { display:grid; grid-template-columns:auto 1fr auto; gap:14px; align-items:center; padding:13px 0; } .action + .action { border-top:1px solid #1a3545; }
...
<body>
  ...
  (() => {
    ...
    const statusNames = { confirmed:"Подтверждено", executed:"Выполнено", clicked:"Выполнено", skipped:"Пропущено", no_op:"Проверка", noop:"Проверка", observe:"Проверка", ambiguous:"Неоднозначно", failed:"Ошибка", error:"Ошибка" };
    ...
    const reasonText = (value) => { const reasons = { "health.low":"Здоровье ниже порога политики", "budget.reserve":"Сохранён резерв зарядов", "cooldown.active":"Действие на перезарядке", "freshness.stale":"Наблюдение устарело", "mode.unknown":"Режим не распознан", "no executable rule matched":"Условия для действия не выполнены", "sequence completed":"Действие выполнено", "fresh diary event and bounded prana change confirm the influence":"Действие подтверждено игрой" }; return reasons[value] || value || "Причина не указана"; };
    const freshnessText = (value) => ({ fresh:"Актуально", stale:"Устарело", expired:"Истекло", unknown:"Данных пока нет", auth_degraded:"Требуется вход" }[String(value)] || "Данных пока нет");
    ...
    const renderAction = (target, raw, clear = true) => { if (clear) target.replaceChildren(); if (!raw || typeof raw !== "object") { const empty = document.createElement("p"); empty.className = "empty"; empty.textContent = "Действий пока нет."; target.append(empty); return; } const state = String(first(raw.status, raw.state, raw.kind, "no_op")).toLowerCase(); const mark = document.createElement("div"); mark.className = "action-mark " + statusClass(state); mark.textContent = state === "ambiguous" ? "!" : state === "skipped" ? "–" : "✓"; const body = document.createElement("div"); body.className = "action-main"; const name = document.createElement("div"); name.className = "action-name"; name.textContent = actionNames[raw.action || raw.command || raw.id] || String(first(raw.action, raw.command, raw.id, "Проверка")); const tag = document.createElement("span"); tag.className = "tag " + statusClass(state); tag.textContent = statusNames[state] || state; name.append(tag); const reason = document.createElement("div"); reason.className = "action-reason"; reason.textContent = reasonText(first(raw.result, raw.reason, raw.reasonCode)); body.append(name, reason); const time = document.createElement("time"); time.className = "action-time"; time.dateTime = String(first(raw.at, raw.createdAt, raw.observedAt, "")); time.textContent = localTime(time.dateTime); const row = document.createElement("div"); row.className = "action"; row.append(mark, body, time); target.append(row); };
    const renderTimeline = (items) => { const target = $("timeline"); target.replaceChildren(); const list = Array.isArray(items) ? items.slice(0, 40) : []; text("cycle-count", list.length ? "Показано " + list.length : "Нет данных"); if (!list.length) { const empty = document.createElement("p"); empty.className = "empty"; empty.textContent = "История появится после первого цикла."; target.append(empty); return; } list.forEach((item) => renderAction(target, item, false)); };
    const render = (payload) => { const data = safeObject(payload); const obs = observationOf(data); const resources = safeObject(data.resources); const identity = safeObject(data.identity); const latest = safeObject(first(data.latestObservation, obs)); const checks = Array.isArray(data.cycles) ? data.cycles : []; const operations = Array.isArray(data.operations) ? data.operations : []; const timelineItems = [...checks, ...operations].sort((a, b) => String(b && b.at || "").localeCompare(String(a && a.at || ""))); const action = first(data.lastAction, data.last_action, operations[0]); const healthPercent = num(first(resources.healthPercent, latest.healthPercent, obs.healthPercent, data.healthPercent)); const pranaCurrent = num(first(resources.prana, latest.prana, safeObject(obs.prana).current, data.prana)); const pranaCapacity = num(first(resources.pranaCapacity, latest.pranaCapacity, safeObject(obs.prana).capacity, data.pranaCapacity)); const charges = num(first(resources.charges, latest.charges, obs.charges, data.charges)); const reserve = num(first(resources.reserveCharges, data.reserveCharges, data.reserve)); const health = renderMetric("health", healthPercent, 100, "%"); $("health-meter").style.width = (health === null ? 0 : health) + "%"; const pranaMeter = renderMetric("prana", pranaCurrent, pranaCapacity); $("prana-meter").style.width = (pranaMeter === null ? 0 : pranaMeter) + "%"; text("charges", charges === null ? "—" : String(Math.round(charges))); text("reserve", reserve === null ? "Резерв: —" : "Резерв: " + Math.round(reserve)); const mode = String(first(latest.mode, obs.mode, data.mode, "unknown")); text("mode", modeNames[mode] || mode); const freshness = String(first(data.freshness, latest.freshness, obs.freshness, "unknown")); text("freshness", "Состояние: " + freshnessText(freshness)); const heroId = first(identity.heroId, latest.heroId, obs.heroId, data.heroId); text("hero-subtitle", heroId ? "Герой: " + heroId : "Только локальное наблюдение"); renderAction($("last-action"), action); const facts = $("facts"); facts.hidden = false; text("observed-at", localTime(first(latest.observedAt, obs.observedAt, data.generatedAt, data.observedAt))); const counts = safeObject(data.counts); text("confirmed-count", String(first(counts.confirmed, 0))); text("ambiguous-count", String(first(counts.ambiguous, 0))); const logs = safeObject(data.logs); const errors = Array.isArray(data.errors) ? data.errors : (Array.isArray(logs.stderr) ? logs.stderr : []); text("error-count", errors.length ? String(errors.length) : "Нет"); renderTimeline(timelineItems.length ? timelineItems : (action ? [action] : [])); const stale = freshness.toLowerCase(); const notice = $("notice"); notice.hidden = true; notice.className = "notice"; if (stale === "stale" || stale === "expired") { notice.hidden = false; notice.classList.add("stale"); notice.textContent = "Наблюдение устарело. Агент не должен принимать решения по этим данным."; } if (errors.length) { notice.hidden = false; notice.className = "notice error"; notice.textContent = "Ошибки последних циклов: " + errors.slice(0, 3).map((error) => String(error).slice(0, 240)).join(" · "); } setStatus(stale === "fresh" ? "ok" : stale === "stale" || stale === "expired" ? "warn" : "", stale === "fresh" ? "Последняя проверка актуальна" : stale === "stale" || stale === "expired" ? "Данные устарели" : "Данных пока нет"); };
